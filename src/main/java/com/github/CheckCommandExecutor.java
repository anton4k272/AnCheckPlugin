...
public class CheckCommandExecutor implements CommandExecutor {
    ...
    private final Set<String> checkedPlayers;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 2 && args[0].equalsIgnoreCase("off")) {
            ...
            if (checkedPlayers.contains(playerName)) {
                removePlayerFromCheck(playerName); // Используем новый метод
    ...
    public void removePlayerFromCheck(String playerName) {
        // Удаляем игрока из списка проверяемых
